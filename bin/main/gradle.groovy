...
def gitMergeMaster(){
	sh "git fetch -p && git checkout main && git merge --no-ff release-${env.NEXT_TAG} && git push"
}

def gitMergeDevelop(){
	sh "git fetch -p && git checkout develop && git merge --no-ff release-${env.NEXT_TAG} && git push"
}

def gitTagMaster(){
	sh "git fetch -p && git checkout main && git tag -a ${env.NEXT_TAG} && git push --tags"
}
